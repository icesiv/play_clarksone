...
async def main():
    async with async_playwright() as p:
        ...
        async def capture_json_response(response):
            request_url = response.request.url
            if 'json' in request_url:
                json_responses.append(response)

        # Intercept and capture the JSON request
        page.on('response', capture_json_response)

        # Wait for the specific JSON request to be captured (you can specify a timeout)
        await asyncio.sleep(5)  # Wait for 5 seconds or adjust as needed

        # Extract and parse the JSON data from the captured responses
 ...
 # ________________________________________________________________________________
        ...
        # load_page(page, item_url)


# def load_page(page, url):
#     full_url = constant.BASE_URL + url
